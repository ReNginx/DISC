...
class LanguageProcessor(nn.Module):
    """Language feature processor using PyTorch's built-in transformer layers.

    Uses learnable queries with transformer decoder to extract fixed-size
    representations from variable-length language sequences.
    """

    def __init__(self, input_dim: int, output_dim: int, num_queries: int = 32, num_heads: int = 8, num_layers: int = 2):
        super().__init__()
        ...
        # Learnable queries (similar to PerceiverIO latents)
        self.queries = nn.Parameter(torch.randn(num_queries, output_dim))

        # Use PyTorch's built-in transformer decoder layer
    ...
    def forward(self, x: torch.Tensor, attention_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        Args:
            ...
            attention_mask: Mask of shape (B, L) where 1=valid, 0=padding

        Returns:
            Output of shape (B, num_queries, output_dim)
        """
        B, L, _ = x.shape

        # Project input to output dimension
        memory = self.input_proj(x)  # (B, L, output_dim)

        # Expand queries for batch
        tgt = self.queries.unsqueeze(0).expand(B, -1, -1)  # (B, num_queries, output_dim)

        # Create memory key padding mask from attention mask
        # PyTorch expects True for positions to ignore
        memory_key_padding_mask = None
        if attention_mask is not None:
            memory_key_padding_mask = attention_mask == 0  # (B, L)

        # Apply transformer decoder
        # memory: (B, L, output_dim) - the input sequence
        # tgt: (B, num_queries, output_dim) - the learnable queries
        output = self.transformer_decoder(tgt=tgt, memory=memory, memory_key_padding_mask=memory_key_padding_mask)

        return output  # (B, num_queries, output_dim)
...
class HyLaPHypoGenPolicy(nn.Module):
    ...
    def __init__(
        ...
        **kwargs: Dict[str, Any],
        ...
        from model.hylap.hypogen2_impl.hypernetworks import HyperTr
