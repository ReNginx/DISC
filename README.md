...
This repo contains only the HyLaP model and the LIBERO / Meta-World pipelines. Baseline
...
├── dataloader/               # LIBERO-original + Meta-World data modules
├── trainer/                  # GenericTrainer + HyLaPTrainer (Lightning)
├── evaluator/                # LIBERO + Meta-World evaluators (with TTT support)
...
mkdir -p third_party
cd third_party
git clone git@github.com:Lifelong-Robot-Learning/LIBERO.git modified_libero
cd ..
...
Train on a different LIBERO suite or on Meta-World:
